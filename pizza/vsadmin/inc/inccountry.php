...
//-->
		  ...
		  <form name="mainform" method="post" action="admincountry.php">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="">
			  ...
			  <tr> 
                <td width="100%" colspan="6"><ul><li>A &quot;Home Country&quot; is one where the states can be selected from the States drop down list on the checkout page.</li></ul></td>
			  </tr>
			  <tr>
				...
				<td align=center><strong><a href="javascript:;" title="<?php print $yyFSApp?>"><?php print $yyFSA?></a></strong></td>
				<td align=center><strong><?php print $yyPosit?></strong></td>
				<td align=center><strong><?php	if($shipType==2 || $shipType==5)
												print $yyPZone;
											else
												print "&nbsp;"; ?></strong></td>
			  </tr><?php
...
<td><input type=checkbox name="ena<?php print $alldata["countryID"]?>" <?php if((int)$alldata["countryEnabled"]==1) print "checked"; ?> /></td>
<td><input type=text name="tax<?php print $alldata["countryID"]?>" value="<?php print (double)$alldata["countryTax"]?>" size="4" />%</td>
<?php print "<script language=JavaScript>wposzon(" . $alldata["countryID"] . "," . $alldata["countryOrder"] . "," . $alldata["countryZone"] . "," . $alldata["countryFreeShip"] . ");</script>" ?>
</tr><?php
		}
	}
?>			  <tr> 
