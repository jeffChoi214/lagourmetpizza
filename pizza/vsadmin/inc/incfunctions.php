...
function do_stock_management($smOrdId){
	global $stockManage;
	if($stockManage != 0){
		$sSQL="SELECT cartID,cartProdID,cartQuantity,pSell FROM cart INNER JOIN products ON cart.cartProdID=products.pID WHERE (cartCompleted=0 OR cartCompleted=2) AND cartOrderID='" . mysql_escape_string(unstripslashes($smOrdId)) . "'";
...
if($doaddprodoptions){
	$sSQL = "SELECT poOptionGroup,optType,optFlags,checkbox FROM prodoptions LEFT JOIN optiongroup ON optiongroup.optGrpID=prodoptions.poOptionGroup WHERE poProdID='" . $rs["pId"] . "' ORDER BY poID";
	$result = mysql_query($sSQL) or print(mysql_error());
...
if(@$noprice!=TRUE && ! ($rs["pPrice"]==0 && @$pricezeromessage != "") && $hasonepriceoption){
	...
	print "var totAdd$Count=0;";
	print 'if(!isW3) return;';
	foreach($prodoptions as $rowcounter => $theopt){
		if(abs($theopt["optType"])!=3){
			if(($theopt["optFlags"]&1)==1)
				print "totAdd$Count=totAdd$Count+((" . $rs["pPrice"] . "*pricechecker(document.forms.tForm" . $Count . ".optn" . $rowcounter . ".options[document.forms.tForm" . $Count . ".optn" . $rowcounter . ".selectedIndex].value))/100.0);\n";
			else
			if ($theopt["checkbox"] != "1")
				print "totAdd$Count=totAdd$Count+pricechecker(document.forms.tForm" . $Count . ".optn" . $rowcounter . ".options[document.forms.tForm" . $Count . ".optn" . $rowcounter . ".selectedIndex].value);\n";
...
if(@$enableclientlogin==TRUE){
	...
	if(@$requiredloginlevel != ""){
		if((int)$requiredloginlevel > @$_SESSION["clientLoginLevel"]){
			...
			header('Location: '.$prot.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']).'/clientlogin.php?refurl=' . urlencode(@$_SERVER["PHP_SELF"] . (@$_SERVER["QUERY_STRING"] !="" ? "?" . @$_SERVER["QUERY_STRING"] : "")));
			exit;
		}
	}
}
