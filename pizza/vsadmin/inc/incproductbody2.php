			  ...
			  <tr>
				<td colspan="<?php print $productcolumns;?>">
				  <table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
					  <td class="prodnavigation" align="<?php print $headeralign;?>"><strong><p class="prodnavigation"><?php print $tslist ?></p></strong></td>
...
?>
			  <tr>
				<td colspan="<?php print $productcolumns;?>" align="center" class="pagenums"><p class="pagenums"><?php
					if($iNumOfPages > 1 && @$pagebarattop==1) print writepagebar($CurPage, $iNumOfPages) . "<br />"; ?>
				  <img src="images/clearpixel.gif" width="300" height="5" alt="" /></p></td>
			  </tr>
<?php
...
?>
              <?php if(($Count % $productcolumns) == 0) print "<tr>" ?>
				<td width="<?php print (int)(100 / $productcolumns);?>%" align="center" valign="top">
				  <table width="100%" border="0" cellspacing="4" cellpadding="4">
				  <?php if(@$showproductid==TRUE) print "<tr><td><strong>" . $xxPrId . ":</strong> " . $rs["pId"] . "</td></tr>"; ?>
				    <tr>
...
<?php updatepricescript(@$noproductoptions!=TRUE); ?>
	<form method="post" name="tForm<?php print $Count;?>" action="cart.php" onsubmit="return formvalidator<?php print $Count;?>(this)">
					<tr>
					  <td width="100%">
					    <p><strong><?php print $startlink . $rs[getlangid("pName",1)] . $endlink . $xxDot;?></strong><?php if($alldiscounts != "") print ' <font color="#FF0000"><strong>' . $xxDsApp . '</strong><br /><font size="1">' . $alldiscounts . "</font></font>" ?><br />
						<?php print $rs[getlangid("pDescription",2)];?><br />
<?php
...
<?php
		...
		if(($Count % $productcolumns) == 0){
			...
			if(! ($Count==$totrows) && $Count < $adminProdsPerPage){
				...
				for($index=1; $index <= $productcolumns; $index++)
					print '<td align="center"><hr width="70%" align="center"></td>';
				print "</tr>";
			}
		}
	}
	if($Count % $productcolumns != 0){
		while($Count % $productcolumns != 0){
			print '<td width="' . (int)(100 / $productcolumns) . '%" align="center">&nbsp;</td>';
			$Count++;
		}
		print "</tr>";
	}
