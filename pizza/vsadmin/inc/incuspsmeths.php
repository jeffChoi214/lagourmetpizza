...
if(@$_POST["posted"]=="1"){
	if($shipType==3){
		...
		if($shipType==6) $indexadd=100; else $indexadd=0;
...
<?php if($shipType!=3 && $shipType!=4 && $shipType!=6){ ?>
        <tr>
          <td width="100%">
            <table width="100%" border="0" cellspacing="0" cellpadding="2" bgcolor="">
			  <tr> 
                <td width="100%" colspan="2" align="center"><br /><strong>This page only applies to UPS, USPS and Canada Post shipping type. Please 
...
<?php		while($allmethods=mysql_fetch_assoc($result)){ ?>
			  <tr>
				...
				<td align="center"><a href="javascript:;" title="<?php print $yyFSApp?>"><?php print $yyFSA?></a></td>
...
<?php		}else{ ?>
			  <tr>
				<td colspan="5"><ul><li><font size="1">You can use this page to set which Canada Post shipping methods qualify for free shipping discounts by checking the FSA (Free Shipping Available) checkbox.</li>
				<li><font size="1"><?php print str_replace("USPS",$yyCanPos,$yyUSS2)?> <a href="http://www.canadapost.ca">http://www.canadapost.ca</a>.</font></li>
				</ul></td>
			  </tr>
<?php		}
			while($allmethods=mysql_fetch_assoc($result)){ ?>
			  <tr>
				...
				<td align="center"><strong><?php print ($shipType==4?$yyUseMet:"&nbsp;")?></strong></td>
				<td align="center"><a href="javascript:;" title="<?php print $yyFSApp?>"><?php print $yyFSA?></a></td>
				<td>&nbsp;</td>
			  </tr>
			  <tr>
				<td colspan="2">&nbsp;</td>
				<td align="center"><input type="<?php print ($shipType==4?"checkbox":"hidden")?>" name="methoduse<?php print $allmethods["uspsID"]?>" value="ON" <?php if((int)$allmethods["uspsUseMethod"]==1) print "checked"?> /></td>
				<td align="center"><input type="checkbox" name="methodfsa<?php print $allmethods["uspsID"]?>" value="ON" <?php if((int)$allmethods["uspsFSA"]==1) print "checked"?> /></td>
				<td>&nbsp;</td>
			  </tr>
			  <tr>
