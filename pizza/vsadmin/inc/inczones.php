...
if(@$_POST["posted"]=="1"){
	...
	print '<meta http-equiv="refresh" content="3; url=adminzones.php">';
	...
	for($index=0; $index <= 59; $index++){
		if(is_numeric(@$_POST["weight" . $index]) && (double)@$_POST["weight" . $index] > 0){
			$sSQL = "INSERT INTO zonecharges (zcZone,zcWeight,zcRate,zcRate2,zcRate3,zcRate4,zcRate5) VALUES (" . $zone . "," . @$_POST["weight" . $index];
			for($index2=0; $index2 < $maxshippingmethods; $index2++){
				if(is_numeric(trim(@$_POST["charge" . $index2 . "x" . $index])))
					$sSQL .= "," . trim(@$_POST["charge" . $index2 . "x" . $index]);
				else
					$sSQL .= ",0";
			}
			mysql_query($sSQL . ')') or print(mysql_error());
	...
	$sSQL = "SELECT zcID,zcWeight,zcRate,zcRate2,zcRate3,zcRate4,zcRate5 FROM zonecharges WHERE zcZone=" . @$_GET["id"] . " ORDER BY zcWeight";
	$result = mysql_query($sSQL) or print(mysql_error());
...
//-->
        ...
        <tr>
		  <form name="mainform" method="post" action="adminzones.php" onsubmit="return formvalidator(this)">
			...
			<?php	for($index=0; $index<=$hasMultiShip;$index++){
						print '<td width="' . (int)(100/(2+$hasMultiShip)) . '%" align="center"><a href="javascript:;" title="'. $yyFSApp . '"><strong>' . $yyFSA . '</strong></a>: <input type="checkbox" value="ON" name="methodfsa' . $index . '" ' . (($pzFSA & pow(2, $index)) != 0 ? "checked" : "") . ' /></td>' . "\r\n";
					}
					for($index=$hasMultiShip+1; $index < $maxshippingmethods; $index++){
...
?>
			...
			<?php	for($index2=0; $index2<$maxshippingmethods; $index2++){
						if($index2 <= $hasMultiShip)
							print '<td align="center"><input type="text" name="charge'. $index2 . "x" . $index . '" value="' . $alldata[$rowcounter][2+$index2] . '" size="14" /></td>' . "\r\n";
						else
...
<?php
        ...
        <tr>
		  ...
		  <td width="100%">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="3" bgcolor="">
			  <tr> 
                <td width="100%" <?php if($splitUSZones) print "colspan='2'";?> align="center"><strong>Use this page to add and modify postal zones.</strong><br />&nbsp;</td>
			  </tr>
			  <tr> 
