...
<?php if(! (@isset($showcategories) && @$showcategories==FALSE)){ ?>
			  <tr>
				<td class="prodnavigation" colspan="2" align="<?php print $headeralign; ?>"><b><p class="prodnavigation"><?php print $tslist ?></p></b></td>
				<td align="right">&nbsp;<?php if(@$nobuyorcheckout != TRUE){ ?><a href="cart.php"><img src="images/checkout.gif" border="0"></a><?php } ?></td>
			  </tr>
<?php }
	...
	if(mysql_num_rows($result2) > 0){ ?>
			  <tr>
				<td align="left" colspan="3">
				  <p><b><?php print $xxDsProd?></b><br><font color="#FF0000" size="1">
				  <?php	while($rs2=mysql_fetch_assoc($result2)){
							print $rs2["cpnWorkingName"] . "<br>";
						} ?></font></p>
				</td>
			  </tr>
<?php
...
?>
			  <tr>
				<td colspan="3" align="center" class="pagenums"><p class="pagenums"><?php
				  ...
				  <img src="images/clearpixel.gif" width="300" height="8"></p></td>
			  </tr>
<?php
	while($rs = mysql_fetch_array($allprods)){
		if(trim($rs["pLongDescription"])!="" || ! (trim($rs["pLargeImage"])=="" || is_null($rs["pLargeImage"]) || trim($rs["pLargeImage"])=="prodimages/")){
			if(@$detailslink != ""){
				$startlink=str_replace('%pid%', $rs["pId"], str_replace('%largeimage%', $rs["pLargeImage"], $detailslink));
				$endlink=@$detailsendlink;
			}else{
				$startlink='<a href="proddetail.php?prod=' . urlencode($rs["pId"]) . (@$catid != "" && @$catid != "0" ? '&cat=' . $catid : "") . '">';
				$endlink="</a>";
			}
		}else{
			$startlink="";
		...
		$alldiscounts = "";
		if(@$noshowdiscounts != TRUE){
			$sSQL = "SELECT DISTINCT cpnWorkingName FROM coupons LEFT OUTER JOIN cpnassign ON coupons.cpnID=cpnassign.cpaCpnID WHERE (cpnSitewide=0 OR cpnSitewide=3) AND cpnNumAvail>0 AND cpnEndDate>='" . date("Y-m-d",time()) ."' AND cpnIsCoupon=0 AND ((cpaType=2 AND cpaAssignment='" . $rs["pId"] . "')";
              ...
              <tr> 
				...
				<?php
					if(trim($rs["pImage"])=="" || is_null($rs["pImage"]) || trim($rs["pImage"])=="prodimages/"){
						...
						print $startlink . '<img src="' . $rs["pImage"] . '" border="0" alt="' . str_replace('"','&nbsp;',$rs["pName"]) . '">' . $endlink;
					}
				?>
                </td>
				<td width="59%">
...
<?php	if(@$showquantonproduct==TRUE){ ?><input type="text" name="quant" size="2" value="1"><?php } ?><input align="absmiddle" type="image" src="images/buy.gif" border="0"><?php
	}else{
		print "<b>".$xxOutStok."</b>";
	}
}			  ?></td>
...
?>
			  <tr>
				<td colspan="3" align="center" class="pagenums"><p class="pagenums"><?php
				  ...
				  <img src="images/clearpixel.gif" width="300" height="1"></p></td>
			  </tr>
			</table>